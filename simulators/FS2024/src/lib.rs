...
pub enum DispatchResult<'a> {
    ...
    EnumerateSimObjectAndLiveryList(&'a SIMCONNECT_RECV_ENUMERATE_SIMOBJECT_AND_LIVERY_LIST),
    FlowEvent(&'a SIMCONNECT_RECV_FLOW_EVENT),
    CameraData(&'a SIMCONNECT_RECV_CAMERA_DATA),
    CameraStatus(&'a SIMCONNECT_RECV_CAMERA_STATUS),
    CameraDefinitionList(&'a SIMCONNECT_RECV_CAMERA_DEFINITION_LIST),
    CommBus(&'a SIMCONNECT_RECV_COMM_BUS),
    CameraWorldLocker(&'a SIMCONNECT_RECV_CAMERA_WORLD_LOCKER),
}
...
impl SimConnector {
    ...
    pub fn request_all_facilities(
        ...
        unsafe { SimConnect_RequestAllFacilities(self.handle, list_type, request_id) == 0 }
    }
    ...
    pub fn camera_acquire(&self, client_id: &str) -> bool {
        let id = cstring(client_id);
        unsafe { SimConnect_CameraAcquire(self.handle, id.as_ptr()) == 0 }
    }

    pub fn camera_release(&self, camera_def_name: &str) -> bool {
        let name = cstring(camera_def_name);
        unsafe { SimConnect_CameraRelease(self.handle, name.as_ptr()) == 0 }
    }

    pub fn camera_get_status(&self) -> bool {
        unsafe { SimConnect_CameraGetStatus(self.handle) == 0 }
    }

    pub unsafe fn camera_set(&self, camera_data: SIMCONNECT_DATA_CAMERA, data_mask: DWORD) -> bool {
        unsafe { SimConnect_CameraSet(self.handle, camera_data, data_mask) == 0 }
    }

    pub fn camera_get(&self, referential: DWORD) -> bool {
        unsafe { SimConnect_CameraGet(self.handle, referential) == 0 }
    }

    pub fn camera_enable_flag(&self, flag: DWORD) -> bool {
        unsafe { SimConnect_CameraEnableFlag(self.handle, flag) == 0 }
    }

    pub fn camera_disable_flag(&self, flag: DWORD) -> bool {
        unsafe { SimConnect_CameraDisableFlag(self.handle, flag) == 0 }
    }

    pub fn subscribe_to_camera_status_update(&self) -> bool {
        unsafe { SimConnect_SubscribeToCameraStatusUpdate(self.handle) == 0 }
    }

    pub fn unsubscribe_to_camera_status_update(&self) -> bool {
        unsafe { SimConnect_UnsubscribeToCameraStatusUpdate(self.handle) == 0 }
    }

    pub fn enumerate_camera_definitions(&self) -> bool {
        unsafe { SimConnect_EnumerateCameraDefinitions(self.handle) == 0 }
    }

    pub fn camera_set_using_camera_definition(&self, camera_definition: &str) -> bool {
        let def = cstring(camera_definition);
        unsafe { SimConnect_CameraSetUsingCameraDefinition(self.handle, def.as_ptr()) == 0 }
    }

    pub fn subscribe_to_camera_world_locker_status_update(&self) -> bool {
        unsafe { SimConnect_SubscribeToCameraWorldLockerStatusUpdate(self.handle) == 0 }
    }

    pub fn unsubscribe_to_camera_world_locker_status_update(&self) -> bool {
        unsafe { SimConnect_UnsubscribeToCameraWorldLockerStatusUpdate(self.handle) == 0 }
    }

    pub unsafe fn request_camera_world_locker(
        &self,
        locker_position: SIMCONNECT_DATA_XYZ,
        referential: SIMCONNECT_POSITION_REFERENTIAL,
        object_id: DWORD,
    ) -> bool {
        unsafe {
            SimConnect_RequestCameraWorldLocker(self.handle, locker_position, referential, object_id) == 0
        }
    }

    pub fn delete_camera_world_locker(&self) -> bool {
        unsafe { SimConnect_DeleteCameraWorldLocker(self.handle) == 0 }
    }

    // ==================== CommBus ====================

    pub fn subscribe_to_comm_bus_event(&self, event_id: SIMCONNECT_CLIENT_EVENT_ID, event_name: &str) -> bool {
        let name = cstring(event_name);
        unsafe { SimConnect_SubscribeToCommBusEvent(self.handle, event_id, name.as_ptr()) == 0 }
    }

    pub fn unsubscribe_to_comm_bus_event(&self, event_id: SIMCONNECT_CLIENT_EVENT_ID) -> bool {
        unsafe { SimConnect_UnsubscribeToCommBusEvent(self.handle, event_id) == 0 }
    }

    pub unsafe fn call_comm_bus_event(
        &self,
        event_name: &str,
        broadcast_to: SIMCONNECT_COMM_BUS_BROADCAST_TO,
        buffer_size: DWORD,
    ...
    pub fn subscribe_to_flow_event(&self) -> bool {
        unsafe { SimConnect_SubscribeToFlowEvent(self.handle) == 0 }
    }

    pub fn unsubscribe_to_flow_event(&self) -> bool {
        unsafe { SimConnect_UnsubscribeToFlowEvent(self.handle) == 0 }
    }
    ...
    pub fn enumerate_sim_objects_and_liveries(
        ...
        unsafe {
            SimConnect_EnumerateSimObjectsAndLiveries(self.handle, request_id, object_type) == 0
        }
    }
    ...
    pub fn get_next_message(&self) -> Result<DispatchResult<'_>, &str> {
        unsafe {
            ...
            match (*data_buf).dwID as SIMCONNECT_RECV_ID {
                ...
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_ENUMERATE_SIMOBJECT_AND_LIVERY_LIST => Ok(DispatchResult::EnumerateSimObjectAndLiveryList(&*(data_buf as *const SIMCONNECT_RECV_ENUMERATE_SIMOBJECT_AND_LIVERY_LIST))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_FLOW_EVENT => Ok(DispatchResult::FlowEvent(&*(data_buf as *const SIMCONNECT_RECV_FLOW_EVENT))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_CAMERA_DATA => Ok(DispatchResult::CameraData(&*(data_buf as *const SIMCONNECT_RECV_CAMERA_DATA))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_CAMERA_STATUS => Ok(DispatchResult::CameraStatus(&*(data_buf as *const SIMCONNECT_RECV_CAMERA_STATUS))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_CAMERA_DEFINITION_LIST => Ok(DispatchResult::CameraDefinitionList(&*(data_buf as *const SIMCONNECT_RECV_CAMERA_DEFINITION_LIST))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_COMM_BUS => Ok(DispatchResult::CommBus(&*(data_buf as *const SIMCONNECT_RECV_COMM_BUS))),
                SIMCONNECT_RECV_ID_SIMCONNECT_RECV_ID_CAMERA_WORLD_LOCKER => Ok(DispatchResult::CameraWorldLocker(&*(data_buf as *const SIMCONNECT_RECV_CAMERA_WORLD_LOCKER))),
